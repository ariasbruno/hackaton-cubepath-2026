...
export const Step2Categories: React.FC<Step2CategoriesProps> = ({
  ...
  return (
    <>
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              className={`p-6 md:p-7 rounded-card border-4 shadow-hard paper-card flex flex-col items-center gap-3 md:gap-4 group transition-all ${
                selectedCategory === cat.id ? `border-${cat.color} bg-${cat.color}-muted` : 'bg-white border-ink/5'
              }`}
            >
              <div
                className={`w-16 h-16 md:w-20 md:h-20 bg-${cat.color}/10 text-${cat.color} rounded-full flex items-center justify-center group-hover:scale-110 transition-transform shadow-inner`}
              >
                <CatIcon className="w-9 h-9 md:w-10 md:h-10" />
              </div>
              <p className="font-display text-lg md:text-xl uppercase tracking-tight">{cat.name}</p>
            </button>
          );
        })}
      </div>
      ...
      {!selectedCategory && (
        <div className="mt-8 bg-ink/5 p-6 md:p-7 rounded-card text-center border-4 border-dashed border-ink/10">
        ...
        <>
          ...
          <div className="hidden md:flex fixed inset-0 bg-ink/50 backdrop-blur-sm z-50 items-center justify-center animate-pure-fade-in">
            <div className="bg-paper w-full max-w-md rounded-card border-4 border-ink/5 shadow-hard-lg p-8 relative animate-scale-in">
              ...
              <button
                onClick={() => setActiveSheetCategory(null)}
                className="absolute top-4 right-4 w-9 h-9 flex items-center justify-center bg-white rounded-full shadow-hard-sm border-2 border-ink/5 active:translate-y-0.5 transition-all"
              >
              ...
              <div className="flex items-center gap-5 mb-8">
                ...
                >
                  {activeCat && CategoryIconMap[activeCat.icon] ? (
                    React.createElement(CategoryIconMap[activeCat.icon], { className: 'w-8 h-8' })
                  ) : (
                    <CategoryIcon className="w-8 h-8" />
                  )}
                </div>
                <div>
                  <h3 className="font-display text-[32px] uppercase tracking-tight" style={{ color: `var(--color-${activeCat?.color})` }}>
                    {activeCat?.name}
                  </h3>
              ...
              <div className="space-y-3 mb-8">
                {activeCat?.subcategories.map((sub) => {
                  ...
                  return (
                    <label
                      key={sub.id}
                      className="flex justify-between items-center bg-white p-5 rounded-btn border-2 border-ink/5 shadow-hard-sm cursor-pointer hover:bg-white transition-all group"
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all group-hover:scale-110 ${isSelected ? 'bg-accent/10 text-accent' : 'bg-ink/5 text-ink/20'}`}>
                          <CheckCircleIcon className="w-5 h-5" />
                        </div>
                        <div>
                          <span className="font-display text-[15px] uppercase tracking-wide leading-none">{sub.name}</span>
                          <p className="text-[9px] font-bold text-ink/40 uppercase leading-none mt-1">Subtema de {activeCat?.name}</p>
                        </div>
                      </div>
                      <input type="checkbox" className="hidden" checked={isSelected} onChange={() => toggleSubcategory(sub.id)} />
                      <div className={`w-14 h-8 rounded-full relative transition-colors shadow-inner ${isSelected ? 'bg-accent' : 'bg-ink/10'}`}>
                        <div className={`w-6 h-6 bg-white rounded-full absolute top-1 shadow-md transition-all ${isSelected ? 'left-7' : 'left-1'}`} />
                      </div>
                    </label>
                  );
                })}
              ...
              <button
                ...
                disabled={selectedSubcategories.length === 0}
                className="w-full bg-primary text-white font-display text-xl py-5 rounded-btn shadow-hard-lg hover:translate-y-0.5 hover:shadow-hard active:translate-y-1 active:shadow-none transition-all uppercase tracking-widest text-center disabled:opacity-50 disabled:pointer-events-none"
              >
                Confirmar
              </button>
            </div>
