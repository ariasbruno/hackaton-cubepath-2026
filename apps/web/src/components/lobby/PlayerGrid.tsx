  ...
  return (
    <section className="flex-1 pb-10">
      <div className="flex justify-between items-end mb-6">
        <h2 className={`font-display text-2xl uppercase tracking-tight text-${accentColor}`}>En la mesa</h2>
        <span className={`bg-${accentColor}/10 text-${accentColor} text-[10px] font-extrabold px-3 py-1 rounded-full border border-${accentColor}/20 uppercase tracking-widest`}>
          {players.length} / {maxPlayers}
        </span>
      </div>

      <div className="grid grid-cols-3 gap-6 pt-2 px-2">
        {players.map((p: any) => {
          ...
          return (
            <div key={p.id} className="flex flex-col items-center gap-2 relative">
              ...
              <span className={`font-bold text-[10px] tracking-tighter text-center truncate w-full ${!p.isReady && !isHost ? 'opacity-40 italic' : ''}`}>
                {p.nickname}{isMe ? ' (Tú)' : ''}
              </span>
            </div>
          );
        ...
        {players.length < maxPlayers && (
          <div className="flex flex-col items-center gap-2 opacity-20">
            <div className="w-20 h-20 bg-ink/5 rounded-full border-4 border-dashed border-ink/10 flex items-center justify-center">
              <PersonAddIcon className="w-12 h-12 text-ink/20" />
            </div>
            <span className="font-bold text-[8px] uppercase tracking-widest">Esperando...</span>
          </div>
        )}
      </div>
    </section>
